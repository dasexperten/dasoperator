...
try{
 ...
 const input={from:'sales@dasexperten.com',to:'customer@example.com',cc:'copy@example.com',bcc:'hidden@example.com',subject:'Проверка 📬',text:'Первая строка\nВторая строка',html:'<p>Текст</p>',inReplyTo:'<parent@example.com>',references:['<first@example.com>','<parent@example.com>'],attachments:[{filename:'договор.bin',contentId:'file@example.com',inline:true,mimeType:'application/octet-stream',content:Buffer.from([0,255,10]).toString('base64')},{filename:'empty.txt',mimeType:'text/plain',content:''}]};
 ...
 assert.equal(mail.inReplyTo,input.inReplyTo);assert.equal(mail.attachments[0].filename,'договор.bin');assert.equal(mail.attachments[0].contentId,'<file@example.com>');assert.equal(mail.attachments[0].disposition,'inline');
