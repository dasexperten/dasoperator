...
ОТВЕТЬ ровно одной строкой: код категории. Ничего больше.`;
  const classifyResult = await callDeepSeek(env, classifySystem, formatInput(input), 2000, 0.0);
...
${skuKnowledge}`;
  ...
  const technologResult = await callDeepSeek(env, technologSystem, technologInput, 4000, 0.1);
