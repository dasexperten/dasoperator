...
// =============================================================================
// GET /api/crm/legacy-customers — Yandex KIT historical snapshot from D1
// =============================================================================
// Stored in crm_legacy_customers table (loaded once from CSV export).
// Independent of Retail CRM. Includes all clients who shopped via Yandex KIT
// before Retail CRM was connected on 2026-04-21.
crm.get('/legacy-customers', async (c) => {
  const page = Math.max(1, Number(c.req.query('page') ?? 1));
  const rawLimit = Number(c.req.query('limit') ?? 50);
  const limit = [20, 50, 100].includes(rawLimit) ? rawLimit : 50;
  ...
  try {
    const totalRow = await c.env.DB.prepare(
      `SELECT COUNT(*) AS n FROM crm_legacy_customers ${where}`
    ).bind(...params).first<{ n: number }>();
    const totalCount = totalRow?.n ?? 0;
    const totalPages = Math.max(1, Math.ceil(totalCount / limit));

    const rowsResp = await c.env.DB.prepare(
      `SELECT external_id, name, phone, email,
              email_consent, registered_at, orders_count, total_spent,
              last_completed_order_at, note, source
       FROM crm_legacy_customers ${where}
       ORDER BY orders_count DESC, total_spent DESC, external_id DESC
       LIMIT ? OFFSET ?`
    ...
    const customers = (rowsResp.results ?? []).map((r) => ({
      ...
      total_spent: r.total_spent / 100, // kopecks → roubles
      last_completed_order_at: r.last_completed_order_at,
      note: r.note,
      source: r.source,
    }));

    return ok(c, {
      source: 'yandex_kit (imported)',
      pagination: {
        page,
        limit,
    ...
    const msg = e instanceof Error ? e.message : 'Unknown error';
    return fail(c, 500, [{ code: 'legacy_db_error', message: msg }]);
  }
});
