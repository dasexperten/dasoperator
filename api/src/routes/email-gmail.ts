...
import { gmailMetadataBatch } from '../lib/gmail-metadata-batch';
...
route.get('/:account/messages', async c => {
  ...
  // One transport for the page, preserving the order returned by Gmail.
  const messages=(await gmailMetadataBatch(token,(page.messages || []).map(m=>m.id))).map(m=>gmailSummary(account,m));
...
route.get('/:account/drafts',async c=>{
  ...
  const source=page.drafts || [];
  const metadata=await gmailMetadataBatch(token,source.map(d=>d.message.id));
  const drafts=source.map((d,i)=>({id:d.id,message:gmailSummary(account,metadata[i])}));
