...
const checks = [
  ...
  [api.includes('getGa4PropertyTimeZone(c.env)') && api.includes('minuteInTimeZone(PRICE_TEST_START_UTC, propertyTimeZone)'), 'price-test UTC seam is converted to the owning GA4 property timezone'],
  ...
  [types.includes('price_test?: {') && types.includes('start_utc: string') && types.includes('property_time_zone: string') && types.includes('end_minute: string'), 'shared type exposes the authoritative UTC and property-local price-test seam'],
