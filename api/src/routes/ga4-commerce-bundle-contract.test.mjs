...
const checks = [
  ...
  [api.includes("cacheKey('ga4:commerce-losses:v6'"), 'commerce cache key invalidates rows without address completion'],
  ...
  [api.includes("'checkout_address_complete'"), 'API requests completed-address events'],
  [ui.includes("checkout_address_complete: 'Delivery address completed'"), 'dashboard labels completed addresses'],
  [ui.includes('addressCompletes / checkoutLoads'), 'dashboard computes address completion from checkout loads'],
  [ui.includes('shippingQuotes / addressCompletes'), 'dashboard computes quote success from completed addresses'],
