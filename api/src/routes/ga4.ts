...
ga4.get('/overview', async (c) => {
  ...
  try {
    const payload = await withKvCache(c.env, cacheKey('ga4:overview:v2', { days, calendar_window: 'exact-v3', host: 'com' }), decisionCacheTtl(days), async () => {
...
ga4.get('/acquisition-detail', async (c) => {
  ...
  try {
    const payload = await withKvCache(
      c.env,
        cacheKey('ga4:acquisition-detail:v6', { days, limit, calendar_window: 'exact-v3', host: 'com' }),
...
ga4.get('/funnel', async (c) => {
  ...
  try {
      const payload = await withKvCache(c.env, cacheKey('ga4:funnel:v3', { days, calendar_window: 'exact-v3', host: 'com' }), decisionCacheTtl(days), async () => {
...
const COMMERCE_LOSS_EVENTS = [
  ...
  'ph_499_add_to_cart',
  'ph_499_cart_view',
  'ph_499_checkout_click',
  'ph_499_purchase_verified',
  'my_2990_price_view',
  'my_2990_add_to_cart',
  'my_2990_cart_view',
  'my_2990_checkout_click',
  'my_2990_purchase_verified',
  'view_cart',
...
ga4.get('/commerce-losses', async (c) => {
  ...
  try {
    const payload = await withKvCache(
      c.env,
      cacheKey('ga4:commerce-losses:v40', { days, limit, decision, calendar_window: 'exact-v3', host: 'com' }),
      ...
      async () => {
        ...
        const price_test = {
          ...
          ph_exact_offer_views: marketPageEventTotal('ph_499_price_view', 'Philippines', '/tl/products/innoweiss'),
          ph_exact_offer_adds: marketPageEventTotal('ph_499_add_to_cart', 'Philippines', '/tl/products/innoweiss'),
          ph_exact_offer_cart_views: marketPageEventTotal('ph_499_cart_view', 'Philippines', '/tl/products/innoweiss'),
          ph_exact_offer_checkout_clicks: marketPageEventTotal('ph_499_checkout_click', 'Philippines', '/tl/products/innoweiss'),
          ph_exact_offer_purchases: totals['ph_499_purchase_verified'] ?? 0,
          my_exact_offer_views: marketPageEventTotal('my_2990_price_view', 'Malaysia', '/ms/products/innoweiss'),
          my_exact_offer_adds: marketPageEventTotal('my_2990_add_to_cart', 'Malaysia', '/ms/products/innoweiss'),
          my_exact_offer_cart_views: marketPageEventTotal('my_2990_cart_view', 'Malaysia', '/ms/products/innoweiss'),
          my_exact_offer_checkout_clicks: marketPageEventTotal('my_2990_checkout_click', 'Malaysia', '/ms/products/innoweiss'),
          my_exact_offer_purchases: totals['my_2990_purchase_verified'] ?? 0,
    ...
    const payload = await withKvCache(c.env, cacheKey('ga4:content:v5', { days, limit, calendar_window: 'exact-v3', host: 'com' }), decisionCacheTtl(days), async () => {
      const [resp, commerce] = await Promise.all([ga4RunReport(c.env, {
        ...
        dimensionFilter: withComHostFilter({ filter: { fieldName: 'eventName', inListFilter: { values: [
          ...
          'shipping_bundle_offer', 'shipping_bundle_view', 'shipping_bundle_add', 'shipping_bundle_unavailable',
...
ga4.get('/realtime', async (c) => {
  ...
  try {
    const commerceEventNames = [
      ...
      'paid_locale_landing_ms', 'paid_locale_landing_zh', 'view_item', 'guide_hero_product_click', 'article_product_click', 'add_to_cart',
      'pdp_value_proof_view', 'pdp_price_view', 'pdp_delivery_preview_ready',
      'view_cart', 'cart_exit_before_checkout', 'shipping_preview_ready', 'shipping_bundle_offer', 'shipping_bundle_view',
      'shipping_bundle_unavailable',
