...
operations.patch('/:id/document-settings', async (c) => {
  ...
  const documentChanged = (Object.keys(parsed.data) as Array<keyof typeof parsed.data>)
    .some((key) => parsed.data[key] !== operation[key as keyof typeof operation]);
  ...
  return ok(c, {
    id: operationId,
    ...parsed.data,
    docs_cancelled: documentChanged,
    updated_at: now,
  }, documentChanged
