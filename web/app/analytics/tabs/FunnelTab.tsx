...
const SIGNAL_LABELS: Record<string, string> = {
  ...
  article_product_click: 'Opened product from expert article',
  add_to_cart: 'Added to cart',
  view_cart: 'Viewed cart',
  cart_exit_before_checkout: 'Exited cart before checkout',
  checkout_cta_click: 'Pressed checkout CTA',
  ...
  checkout_error_ru_phone: 'RU error · phone missing',
  checkout_error_ru_email_missing: 'RU error · email missing',
  checkout_error_ru_email_invalid: 'RU error · email invalid',
  checkout_error_ru_consent: 'RU error · consent missing',
  checkout_error_ru_stock: 'RU error · out of stock',
  checkout_error_ru_pickup: 'RU error · pickup point missing',
  checkout_error_ru_empty_cart: 'RU error · empty cart',
  checkout_error_ru_service: 'RU error · checkout service',
...
export default function FunnelTab() {
  ...
  const phExactOfferPurchases = priceTestLosses.data?.price_test?.ph_exact_offer_purchases ?? 0;
  const phExactOfferPurchaseRate = phExactOfferViews > 0 ? (phExactOfferPurchases / phExactOfferViews) * 100 : null;
  ...
  const myExactOfferPurchases = priceTestLosses.data?.price_test?.my_exact_offer_purchases ?? 0;
  const myExactOfferPurchaseRate = myExactOfferViews > 0 ? (myExactOfferPurchases / myExactOfferViews) * 100 : null;
      ...
      <Panel title="Paid landing through checkout — progress, failures and handoffs" source="Google Ads calendar delivery + GA4 post-launch events">
        ...
        {losses.data && (
          <>
            ...
            <div className="wa-kpis" style={{ marginBottom: 16 }}>
              <Kpi label="PH exact offer → paid" value={fmtPct(phExactOfferPurchaseRate)} delta={`${fmtNum(phExactOfferPurchases)} Stripe-verified purchases ÷ ${fmtNum(phExactOfferViews)} verified ₱499 views`} />
              <Kpi label="MY exact offer → paid" value={fmtPct(myExactOfferPurchaseRate)} delta={`${fmtNum(myExactOfferPurchases)} Stripe-verified purchases ÷ ${fmtNum(myExactOfferViews)} verified RM29.90 views`} />
              <Kpi accent label="Verified price-test sales" value={fmtNum(phExactOfferPurchases + myExactOfferPurchases)} delta="DE210-only · exact goods price · shipping excluded" />
            </div>
        ...
        {pageTotals.length > 0 && (
          <div className="wa-table-scroll" style={{ marginBottom: 16 }}>
            <table className="wa-table">
              <thead>
                <tr><th>Page · complete 30d aggregate</th><th>Country</th><th>Price views</th><th>Add to cart</th><th>Price → cart</th><th>Viewed cart</th><th>Exited before checkout</th><th>Pressed checkout</th><th>Checkout opened</th><th>People: cart → checkout</th><th>Errors</th></tr>
              </thead>
              <tbody>
                {pageTotals.slice(0, 25).map((row) => {
                  const rate = row.price_views > 0 ? (row.add_to_cart / row.price_views) * 100 : null;
                  const checkoutRate = row.view_cart_users > 0 ? (row.begin_checkout_users / row.view_cart_users) * 100 : null;
                  return (
                    <tr key={`${row.country}-${row.page}`}>
                      ...
                      <td className="num">{fmtNum(row.view_cart)} events · {fmtNum(row.view_cart_users)} people</td>
                      <td className="num">{fmtNum(row.cart_exit_before_checkout)} events · {fmtNum(row.cart_exit_before_checkout_users)} people</td>
                      <td className="num">{fmtNum(row.checkout_cta_click)}</td>
                      <td className="num">{fmtNum(row.begin_checkout)} events · {fmtNum(row.begin_checkout_users)} people</td>
