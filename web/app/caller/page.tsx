...
export default function CallerPage() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    setError(null);
    const res = await apiGet<{ calls: CallRow[] }>('/api/calls?limit=200');
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-extrabold uppercase text-stone-500">Emailer · voice</p>
          <h1 className="mt-1 text-3xl font-extrabold text-foreground">Caller</h1>
          <p className="mt-2 max-w-2xl text-base font-semibold text-muted-foreground">
            Every agent call with its transcript. Click a call to read the full conversation.
          </p>
        </div>
        <button
          type="button"
