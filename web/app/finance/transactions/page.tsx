...
import { Loader2, ChevronLeft, ArrowDownLeft, ArrowUpRight, CheckCircle2, AlertCircle, Search } from 'lucide-react';
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (accRes.success && accRes.result) {
          setAccounts(accRes.result.accounts);
        }
        setError(null);
      } catch (e) {
  ...
  return (
    <div className="px-8 py-6 max-w-screen-2xl">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <select
          value={matchedFilter}
          onChange={(e) => setMatchedFilter(e.target.value as 'all' | 'matched' | 'unmatched')}
          style={{
            border: '1px solid var(--line-1)',
            borderRadius: 'var(--radius-sm)',
            backgroundColor: 'var(--paper)',
      ...
      {!loading && transactions.length === 0 && (
        <div style={{ fontSize: '14px', color: 'var(--fg-2)' }}>
          No transactions yet. Run a sync from Bank Accounts page to pull history.
        </div>
      )}
    </div>
  );
