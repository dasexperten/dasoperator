  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      {scenarios.map((s) => {
        ...
        const className = isRecommended ? 'border-2 border-blue-500 bg-blue-50' : 'border border-stone-200 bg-white';
        const isOutOfBounds = (s.mode === 'pallet' && s.pallets > 6) || (s.mode === '40ft' && s.fill_pct < 60);
        return (
          <div key={s.mode} className={`rounded-lg p-4 ${className}`} style={{ opacity: isOutOfBounds && !isRecommended ? 0.55 : 1 }}>
            <div className={`flex items-center gap-2 ${isRecommended ? 'text-blue-700' : 'text-stone-500'}`} style={{ fontSize: '12px', fontWeight: 500, textTransform: 'uppercase' }}>
              <Icon className="w-3.5 h-3.5" />{LABELS[s.mode]}{isRecommended && <span className="ml-1">★</span>}
            </div>
            <div className="mt-2" style={{ fontSize: '14px', fontWeight: 500 }}>{s.total_units.toLocaleString()} units · {s.total_volume_m3.toFixed(1)} m³</div>
            <div className="text-stone-500" style={{ fontSize: '12px', marginTop: '2px' }}>
              {s.mode === 'pallet' ? `${s.pallets} pallets` + (s.pallets > 6 ? ' · over limit (max 6)' : '') : `${s.fill_pct}% filled`}
            </div>
          </div>
        );
      })}
