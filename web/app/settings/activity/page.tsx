  ...
  useEffect(() => {
    ...
    apiGet<DashboardData>('/api/activity/dashboard')
      .then((r) => {
        ...
        else setError(r.errors?.[0]?.message ?? 'Не удалось загрузить данные');
      })
  ...
  return (
    <div className="px-8 py-6 max-w-screen-2xl">
      ...
      <p style={{ fontSize: '14px', color: 'var(--fg-2)', marginBottom: '20px', maxWidth: '720px' }}>
        Последние 7 дней. Рабочее окно 10:00–18:00, Пн–Пт (выходные не влияют на средний день и % активности).
        Активность — доля рабочего окна, когда мышь/клавиатура были в движении.
      </p>
      ...
      {isAdmin && loading && (
        <div style={{ color: 'var(--fg-3)', fontSize: '14px' }}>Загрузка…</div>
      )}
      ...
      {isAdmin && !loading && data && (
        <>
          <div className="flex flex-wrap gap-4 mb-3" style={{ fontSize: '12px', color: 'var(--fg-2)' }}>
            <span className="inline-flex items-center gap-1.5"><span style={{ width: 10, height: 10, borderRadius: 2, background: '#1D9E75', display: 'inline-block' }} />Активное время</span>
            <span className="inline-flex items-center gap-1.5"><span style={{ width: 10, height: 10, borderRadius: 2, background: '#B4B2A9', display: 'inline-block' }} />Пассивно (вкладка открыта)</span>
          </div>
          ...
          <div style={{ border: '1px solid var(--line-1)', borderRadius: 'var(--radius-md)', backgroundColor: 'var(--paper)', overflow: 'hidden', maxWidth: '900px' }}>
            {data.users.length === 0 && (
              <div style={{ padding: '20px', color: 'var(--fg-3)', fontSize: '14px' }}>Пока нет данных активности.</div>
            )}
            {data.users.map((u, i) => {
              ...
              return (
                <div key={u.id} style={{ borderTop: i === 0 ? 'none' : '1px solid var(--line-1)' }}>
                  ...
                  >
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--fg-2)', width: 96 }}>{u.week_hours.toFixed(1)} ч / нед</span>
                    <span style={{ fontSize: '12px', color: 'var(--fg-2)', width: 92 }}>{u.avg_day_hours.toFixed(1)} ч / день</span>
                  ...
                  {open && (
                    <div style={{ padding: '4px 16px 18px' }}>
                      <div className="grid gap-2.5 mb-3.5" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))' }}>
                        {[
                          { l: 'В системе за неделю', v: `${u.week_hours.toFixed(1)} ч` },
                          { l: 'Средний день (Пн–Пт)', v: `${u.avg_day_hours.toFixed(1)} ч` },
                          { l: 'Активность', v: `${u.activity_pct}%` },
                        ].map((m) => (
                        ...
                        {u.daily.map((d, di) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span style={{ fontSize: '11px', color: d.is_weekday ? 'var(--fg-2)' : 'var(--fg-3)', fontWeight: isSel ? 700 : 400 }}>{WEEKDAY_RU[d.weekday]}</span>
                            </button>
                          );
                        })}
                      </div>
                      ...
                      <div style={{ marginTop: 10, fontSize: '13px', color: 'var(--fg-2)', display: 'flex', alignItems: 'center', gap: 8 }}>
                        {sel === null ? (
                          <>Нажми на столбец дня — покажу первый заход и время в системе.</>
                        ) : (() => {
                          const d = u.daily[sel];
                          if (!d || d.in_system_h <= 0) {
                            return <><CalendarOff className="h-4 w-4" /><span><b>{WEEKDAY_RU[d?.weekday ?? 0]} {d?.date}</b> — заходов не было</span></>;
                          }
                          return <><LogIn className="h-4 w-4" style={{ color: '#1D9E75' }} /><span><b>{WEEKDAY_RU[d.weekday]} {d.date}</b> · первый заход <b>{d.first_login}</b> · в системе <b>{d.in_system_h.toFixed(1)} ч</b> (активно {d.active_h.toFixed(1)} ч)</span></>;
                        })()}
                      </div>
                    </div>
                  )}
