...
function GmailComposer({ account, identities, initial, onClose, onSaved }: { account: string; identities: string[]; initial: Compose; onClose: () => void; onSaved: () => void }) {
  ...
  const loadOriginal = useCallback(async () => {
    ...
    try {
      ...
      if(initial.mode === 'draft') {setOriginalHtml(m.html);setBodyEdited(false);setFrom(addresses(m.from)[0] || account);setTo(m.to || '');setCc(m.cc || '');setBcc(m.bcc || '');setSubject(m.subject || '');setText(m.text || (m.html ? new DOMParser().parseFromString(m.html,'text/html').body.textContent || '' : ''));setThread({gmailThreadId:m.gmailThreadId,inReplyTo:m.inReplyTo,references:m.references});}
  ...
  useEffect(() => {const previous=document.activeElement as HTMLElement|null;dialog.current?.focus();const warn=(e:BeforeUnloadEvent)=>{e.preventDefault();e.returnValue='';};window.addEventListener('beforeunload',warn);return()=>{window.removeEventListener('beforeunload',warn);previous?.focus();};},[]);
  ...
  const save=async(send:boolean)=>{
    ...
    try {const r=await gmailSave(account,{id:draftId,from,to,cc,bcc,subject,text,html:bodyEdited ? undefined : originalHtml,...thread,attachments:files.map(({filename,mimeType,content,contentId,inline})=>({filename,mimeType,content,contentId,inline}))});setDraftId(r.draft.id);if(send){sendStarted=true;await gmailSend(account,r.draft.id);}onSaved();}
  ...
  return <div className="gm-modal-backdrop"><div className="gm-modal" role="dialog" aria-modal="true" aria-labelledby="gm-compose-title" tabIndex={-1} ref={dialog} onKeyDown={e=>{
    if(e.key==='Escape'){e.preventDefault();closeRef.current();}
    if(e.key==='Tab'){const controls=Array.from(dialog.current?.querySelectorAll<HTMLElement>('button:not(:disabled),input:not(:disabled),textarea:not(:disabled),select:not(:disabled),a[href]') || []);const first=controls[0],last=controls[controls.length-1];if(!first){e.preventDefault();return;}if(document.activeElement===dialog.current){e.preventDefault();(e.shiftKey?last:first)?.focus();}else if(e.shiftKey && document.activeElement===first){e.preventDefault();last?.focus();}else if(!e.shiftKey && document.activeElement===last){e.preventDefault();first.focus();}}
  }}><div className="gm-compose-header"><h2 id="gm-compose-title">{initial.mode==='draft'?'Черновик Gmail':'Новое письмо'}</h2><button disabled={busy || loading} aria-label={ready && !uncertain?'Сохранить и закрыть':'Закрыть'} onClick={()=>closeRef.current()}><X size={20}/></button></div>
    ...
    <fieldset disabled={!ready || busy || uncertain}><label>От кого<select value={from} onChange={e=>setFrom(e.target.value)}>{Array.from(new Set([from,...identities])).map(a=><option key={a}>{a}</option>)}</select></label><label>Кому<input value={to} onChange={e=>setTo(e.target.value)} placeholder="Адреса через запятую"/></label><label>Копия (CC)<input value={cc} onChange={e=>setCc(e.target.value)}/></label><label>Скрытая копия (BCC)<input value={bcc} onChange={e=>setBcc(e.target.value)}/></label><label>Тема<input value={subject} onChange={e=>setSubject(e.target.value)}/></label><label>Текст<textarea rows={8} value={text} onChange={e=>{setText(e.target.value);setBodyEdited(true);}}/></label><label>Прикрепить файлы<input type="file" multiple onChange={e=>{void upload(e.target.files);e.target.value='';}}/></label><small>До 20 файлов · 10 МБ на файл · 20 МБ всего</small><div className="gm-compose-files">{files.map((f,i)=><div key={`${i}:${f.filename}`}><span>{f.filename}<small>{fileSize(f.size)}</small></span><button type="button" aria-label={`Удалить вложение ${f.filename}`} onClick={()=>setFiles(prev=>prev.filter((_,j)=>j!==i))}>Удалить</button></div>)}</div></fieldset>
    <div className="gm-compose-actions">{uncertain?<button onClick={onClose}>Закрыть и проверить отправку</button>:<><button className="gm-primary" disabled={!ready || busy} onClick={()=>void save(true)}><Send size={16}/>{busy?'Сохранение…':'Отправить'}</button><button disabled={!ready || busy} onClick={()=>void save(false)}>Сохранить и закрыть</button></>}</div>
  </div></div>;
}
