...
export default function InboxView() {
  ...
  async function fileThread(threadId: string) {
    setFiling(threadId);
    try {
      const r = await apiPost<{ count: number; filed: Array<{ status?: string; classification?: string; operation?: string | null }> }>('/api/inbox/ingest-thread', { thread_id: threadId });
      let label = 'File failed';
      if (r.success && r.result) {
        const f = r.result.filed || [];
        if (!f.length) {
          label = 'No documents';
        } else {
          const top = f.find((x) => x.operation) || f[0];
          if (top.operation) label = `Filed → ${top.operation}`;
          else if (top.status === 'already_filed') label = 'Already filed';
          else if (top.status === 'rejected') label = 'Not an invoice';
          else label = `Filed: ${top.classification || 'document'}`;
        }
      }
      setFileResult((p) => ({ ...p, [threadId]: label }));
    } catch {
      setFileResult((p) => ({ ...p, [threadId]: 'File failed' }));
    }
    setFiling(null);
  }
            ...
            <div key={t.thread_id} className={`flex items-start gap-4 px-4 py-3 border-b border-border last:border-b-0 border-l-4 ${cat.spine} hover:bg-muted/40`}>
              ...
              <div className="flex flex-col items-end gap-2 shrink-0">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {t.has_attachments && (
                    <button
                      disabled={filing === t.thread_id}
                      onClick={() => fileThread(t.thread_id)}
                      title="File into ERP — classify the document and match it to an operation"
                      className="text-xs rounded-md px-2.5 py-1 inline-flex items-center gap-1 border border-border hover:bg-muted disabled:opacity-50">
