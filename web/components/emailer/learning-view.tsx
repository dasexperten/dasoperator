...
export default function LearningView() {
  ...
  return (
    <div className="space-y-6">
      <div className={`rounded-lg px-4 py-3 border-l-4 ${autoLearning ? 'bg-emerald-50 border-l-emerald-500' : 'bg-muted/40 border-l-muted-foreground/30'}`}>
        ...
        <p className="text-sm text-muted-foreground mt-2">
          {autoLearning
            ? 'ON — repeated, confident lessons (seen ≥3×) apply on their own. The system stops asking you; you can still revert any from the playbook below.'
          ...
          {lessons.map((l) => (
            <div key={l.id} className="rounded-lg border border-border bg-card p-4 border-l-4 border-l-indigo-400 shadow-md">
              <div className="flex flex-wrap items-center gap-1.5 mb-2">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700">Hermes</span>
              </div>
              <div className="text-[11px] font-medium uppercase text-indigo-600 tracking-wide">What it understood</div>
              ...
              {(l.diff_before || l.diff_after) && (
                <div className="rounded-md border border-border overflow-hidden mb-3 text-base">
                  ...
                  <div className="px-3 py-2 bg-emerald-50 border-t border-border">
                    <div className="text-[11px] font-medium uppercase text-emerald-600">You sent — so the rule follows you</div>
      ...
      <div>
        ...
        <div className="rounded-lg border border-border overflow-hidden">
          ...
          {playbook.map((p) => (
            <div key={p.id} className="flex items-start justify-between gap-3 px-4 py-3 border-b border-border last:border-b-0 border-l-4 border-l-emerald-500">
              <div>
              ...
              <button onClick={() => revert(p.id)} disabled={busy === p.id} className="text-xs text-muted-foreground bg-muted rounded px-2.5 py-1.5 inline-flex items-center gap-1 shrink-0 disabled:opacity-50 hover:bg-muted/70">
