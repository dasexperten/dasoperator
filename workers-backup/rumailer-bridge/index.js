...
var index_default = {
  async fetch(request, env2, ctx) {
    ...
    if (pathname === "/health") {
      return successResponse({ status: "ok", service: "rumailer-bridge", provider: "resend" });
    }
    if (pathname === "/send" && request.method === "POST") {
      return handleSendEmail(request, env2, ctx);
    }
    if (pathname === "/status" && request.method === "GET") {
      return handleGetStatus(request, env2);
    }
    return errorResponse(404, "Not Found");
  }
};
async function handleSendEmail(request, env2, ctx) {
  try {
    ...
    const { from, to, cc, bcc, subject, body, plain_text, attachments, template_id, recipient_id, priority } = payload;
    ...
    if (plain_text) resendPayload.text = plain_text;
    ...
    if (!resendResponse.ok) {
      ...
      return errorResponse(500, `Failed to send: ${resendData.message || "Unknown error"}`);
    }
    const resendId = resendData.id;
    const r2Key = generateR2Key(messageId);
    await tryUploadToR2(env2, r2Key, {
      message_id: messageId,
      resend_id: resendId,
      from,
      to,
      ...
      body,
      plain_text,
      attachments: attachments ? attachments.map((a) => ({ filename: a.filename, mime_type: a.mime_type })) : null,
      template_id,
      recipient_id,
    ...
    await tryLogToD1(env2, {
      ...
      from_address: from,
      to_address: Array.isArray(to) ? to.join(",") : to,
      cc_addresses: cc ? Array.isArray(cc) ? cc.join(",") : cc : null,
      bcc_addresses: bcc ? Array.isArray(bcc) ? bcc.join(",") : bcc : null,
      subject,
      template_id,
      recipient_id,
      priority: priority || "normal",
    ...
    console.error("Send error:", error3);
    return errorResponse(500, `Internal error: ${error3.message}`);
  }
}
__name(handleSendEmail, "handleSendEmail");
async function handleGetStatus(request, env2) {
  try {
    const url = new URL(request.url);
    const messageId = url.searchParams.get("message_id");
    if (!messageId) return errorResponse(400, "message_id required");
    if (env2.DB) {
      const result = await env2.DB.prepare(
        "SELECT * FROM emails_sent WHERE message_id = ? LIMIT 1"
      ).bind(messageId).first();
      if (result) return successResponse(result);
    }
    return errorResponse(404, `Email not found: ${messageId}`);
  } catch (error3) {
    return errorResponse(500, error3.message);
  }
}
__name(handleGetStatus, "handleGetStatus");
function validatePayload(payload) {
  const { from, to, subject, body } = payload;
  if (!from || typeof from !== "string") return { valid: false, error: "from is required" };
  if (!from.includes("@dasexperten.ru") && !from.includes("@send.dasexperten.ru")) return { valid: false, error: "from must be @dasexperten.ru or @send.dasexperten.ru address" };
  if (!to) return { valid: false, error: "to is required" };
  if (!subject) return { valid: false, error: "subject is required" };
  if (!body) return { valid: false, error: "body is required" };
  return { valid: true };
}
__name(validatePayload, "validatePayload");
async function tryUploadToR2(env2, key, data) {
  if (!env2.R2_BUCKET) return false;
  try {
    await env2.R2_BUCKET.put(key, JSON.stringify(data, null, 2), {
      contentType: "application/json"
    });
    return true;
  } catch (error3) {
    console.error("R2 error:", error3);
    return false;
  }
}
__name(tryUploadToR2, "tryUploadToR2");
async function tryLogToD1(env2, data) {
  ...
  try {
    ...
    await env2.DB.prepare(
      `INSERT INTO emails_sent (${cols.join(",")}) VALUES (${placeholders})`
    ).bind(...values).run();
    return true;
  } catch (error3) {
    console.error("D1 error:", error3);
    return false;
  }
}
__name(tryLogToD1, "tryLogToD1");
function generateR2Key(messageId) {
  const date = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  return `emails/sent/${date}/${messageId}.json`;
}
